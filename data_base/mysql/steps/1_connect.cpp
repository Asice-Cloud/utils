...
class MySQLDatabase {
public:
  MySQLDatabase(const char *host, const char *user, const char *password,
                const char *dbname) {
...
int main() {
  ...
  simdjson::padded_string json_data = std::string(R"({"key": "value"})");
  simdjson::dom::element element = parser.parse(json_data);
