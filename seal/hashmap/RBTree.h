...
template <typename K, typename V> struct RBNode {
  ...
  RBNode(K k, V v)
      : key(k), value(v), color(Color::RED), left(nullptr), right(nullptr),
        parent(nullptr) {}
};
...
private:
  ...
  void
  inorderTraversalHelper(RBNode<K, V> *node,
...
template <typename K, typename V> bool RBTree<K, V>::remove(const K &key) {
  RBNode<K, V> *z = root;
  while (z != nullptr) {
    if (key == z->key) {
      break;
...
template <typename K, typename V>
void RBTree<K, V>::inorderTraversalHelper(
    RBNode<K, V> *node, std::function<void(const K &, const V &)> func) const {
  if (node != nullptr) {
