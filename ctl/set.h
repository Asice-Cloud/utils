...
static inline B *JOIN(A, begin)(A *self) { return self->root; }
...
static inline B *JOIN(B, min)(B *self) {
  while (self->l)
    self = self->l;
  return self;
}

static inline B *JOIN(B, max)(B *self) {
  while (self->r)
    self = self->r;
  return self;
}
...
static inline int JOIN(B, color)(B *self) { return self ? self->color : 1; }

static inline int JOIN(B, is_blk)(B *self) { return JOIN(B, color)(self) == 1; }

static inline int JOIN(B, is_red)(B *self) { return JOIN(B, color)(self) == 0; }

static inline B *JOIN(B, grandfather)(B *self) { return self->p->p; }

static inline B *JOIN(B, sibling)(B *self) {
  if (self == self->p->l)
    return self->p->r;
  else
    return self->p->l;
...
static inline B *JOIN(B, uncle)(B *self) { return JOIN(B, sibling)(self->p); }
...
static inline B *JOIN(A, lower_bound)(A *self, T key) {
  B *node = self->root;
  B *result = NULL;
  while (node) {
...
static inline B *JOIN(A, upper_bound)(A *self, T key) {
  B *node = self->root;
  B *result = NULL;
  while (node) {
    int diff = self->compare(&key, &node->key);
...
static inline B *JOIN(A, find)(A *self, T key) {
  B *node = self->root;
  while (node) {
    int diff = self->compare(&key, &node->key);
    if (diff == 0)
...
static inline void JOIN(B, verify_property_1)(B *self) {
...
static inline void JOIN(B, verify_property_2)(B *self) {
  assert(JOIN(B, is_blk)(self));
}

static inline void JOIN(B, verify_property_4)(B *self) {
...
static inline void JOIN(B, count_blk)(B *self, int nodes, int *in_path) {
...
static inline void JOIN(B, verify_property_5)(B *self) {
  int in_path = -1;
...
static inline void JOIN(A, insert_1)(A *, B *), JOIN(A, insert_2)(A *, B *),
    JOIN(A, insert_3)(A *, B *), JOIN(A, insert_4)(A *, B *),
    JOIN(A, insert_5)(A *, B *);
...
static inline void JOIN(A, erase_3)(A *self, B *node) {
  if (JOIN(B, is_blk)(node->p) && JOIN(B, is_blk)(JOIN(B, sibling)(node)) &&
...
static inline void JOIN(A, erase_4)(A *self, B *node) {
  if (JOIN(B, is_red)(node->p) && JOIN(B, is_blk)(JOIN(B, sibling)(node)) &&
      JOIN(B, is_blk)(JOIN(B, sibling)(node)->l) &&
      JOIN(B, is_blk)(JOIN(B, sibling)(node)->r)) {
    JOIN(B, sibling)(node)->color = 0;
    node->p->color = 1;
...
static inline void JOIN(A, erase_5)(A *self, B *node) {
  if (node == node->p->l && JOIN(B, is_blk)(JOIN(B, sibling)(node)) &&
      JOIN(B, is_red)(JOIN(B, sibling)(node)->l) &&
      JOIN(B, is_blk)(JOIN(B, sibling)(node)->r)) {
    JOIN(B, sibling)(node)->color = 0;
    JOIN(B, sibling)(node)->l->color = 1;
    JOIN(A, rotate_r)(self, JOIN(B, sibling)(node));
  } else if (node == node->p->r && JOIN(B, is_blk)(JOIN(B, sibling)(node)) &&
