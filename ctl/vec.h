...
static inline T *JOIN(A, begin)(A *self) { return JOIN(A, front)(self); }

static inline T *JOIN(A, end)(A *self) { return JOIN(A, back)(self) + 1; }
...
static inline Z JOIN(Z, range)(A *container, T *begin, T *end)
{
  (void)container;
  static Z zero;
  Z self = zero;
...
static inline void JOIN(A, reserve)(A *self, const size_t capacity)
{
...
static inline void JOIN(A, insert)(A *self, size_t index, T value)
{
  if (self->size > 0)
  {
...
static inline void JOIN(A, erase)(A *self, size_t index)
{
  static T zero;
...
static inline void JOIN(A, ranged_sort)(A *self, int64_t a, int64_t b,
                                        int _compare(T *, T *))
{
  if (a >= b)
    return;
...
static inline T *JOIN(A, find)(A *self, T key, int _equal(T *, T *))
{
  foreach (A, self, it)
