...
int main(int argc, char *argv[]) {
    ...
    msgBox.setText("Do you want to enable Surprise Mode? (Portals will appear "
                   "if enabled)");
    ...
    QObject::connect(mainWindow, &LabyrinthWidget::requestOpenPortal, [&]() {
        ...
        portalWindow->setReturnPosition(mainWindow->getPortalBRow(),
                                        mainWindow->getPortalBCol());
        portalWindow->show();
        QObject::connect(
            portalWindow, &LabyrinthWidget::requestReturnFromPortal, mainWindow,
            [&](int row, int col) {
