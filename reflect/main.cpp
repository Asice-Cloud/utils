...
public:
        ...
        : name(n), age(a), height(170.0), is_employed(false)
    {
        ...
        REGISTER_FUNCTIONS(FUNCTION(introduce), FUNCTION(get_info), FUNCTION(set_age), FUNCTION(calculate_bmi));
    }
    ...
    double calculate_bmi(double weight, bool use_metric = true)
    {
        ...
        std::cout << "BMI calculation: " << weight << "kg / (" << height_m << "m)² = " << bmi << std::endl;
        return bmi;
    }
};
...
public:
    ...
    void func0()
    {
        std::cout << "func0() - no parameters" << std::endl;
    }
    ...
    std::string func2(int x, const std::string &s)
    {
        ...
        std::cout << "func2(" << x << ", \"" << s << "\") - returns \"" << result << "\"" << std::endl;
        return result;
    }
...
int main()
{
    std::cout << "🚀 C++ Variadic Template Reflection System Demo 🚀" << std::endl;
    ...
    std::cout << "Name: " << std::any_cast<std::string>(*p.get_property("name")) << std::endl;
    std::cout << "Age: " << std::any_cast<int>(*p.get_property("age")) << std::endl;
    std::cout << "Employed: " << (std::any_cast<bool>(*p.get_property("is_employed")) ? "Yes" : "No") << std::endl;
    ...
    try
    {
        ...
        auto result5 = demo.call_function("func5", {7, 1.5, std::string("complex"), false, 9.9f});
    }
    ...
    try
    {
        ...
        std::cout << "Node info: " << std::any_cast<std::string>(node_info) << std::endl;

        n.call_function("set_value", {200});

        auto calc_result = n.call_function("calculate", {2.0, 50.0});
        std::cout << "Calculation result: " << std::any_cast<double>(calc_result) << std::endl;
        ...
        std::cout << "Complex result: \"" << std::any_cast<std::string>(complex_result) << "\"" << std::endl;
    }
    catch (const std::exception &e)
    {
        std::cerr << "❌ Error: " << e.what() << std::endl;
    ...
    std::cout << "Using visit_all_members to inspect person object:" << std::endl;
    p.visit_all_members(
        // 属性访问者
    ...
    std::cout << "✅ Property reflection: Dynamic get/set with type safety" << std::endl;
    std::cout << "✅ Function reflection: Support for 0-N parameters with variadic templates" << std::endl;
    std::cout << "✅ Unified API: Single register_function for all parameter counts" << std::endl;
    std::cout << "✅ Type safety: Compile-time type checking with runtime conversion" << std::endl;
    std::cout << "✅ Visitor pattern: Traverse all members programmatically" << std::endl;
    std::cout << "✅ Batch registration: REGISTER_MEMBERS/REGISTER_FUNCTIONS macros" << std::endl;

    std::cout << "\n🎉 Reflection system demo completed successfully!" << std::endl;
