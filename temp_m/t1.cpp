...
public:
  template <typename T> using container_type = Container<T, std::allocator<T>>;

  template <typename T> void demonstrate() {
...
// 模板模板参数示例 2：策略模式
template <template <typename> class Policy> class ConfigurableClass {
public:
  template <typename T> void apply_policy(T value) {
    Policy<T> policy;
...
// 策略类示例
template <typename T> class PrintPolicy {
public:
  void execute(T value) { std::cout << "Printing: " << value << std::endl; }
};

template <typename T> class DoublePolicy {
public:
  void execute(T value) { std::cout << "Doubling: " << value * 2 << std::endl; }
};
